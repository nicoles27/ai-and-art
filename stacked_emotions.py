...
# Ensure 'time_seconds' is an integer
df["time_seconds"] = df["time_seconds"].astype(int)

# Sort values to ensure proper ordering
df = df.sort_values(by="time_seconds")

# Create a new column for sequential numbers for the x-axis
df['time_index'] = range(1, len(df) + 1)

# Count occurrences of each emotion per second
pivoted = df.pivot_table(
    index="time_index",  # Use the new sequential time_index for the x-axis
...
# Set x-axis labels to increasing integers with more space between them
